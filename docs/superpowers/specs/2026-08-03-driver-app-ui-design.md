...
| `useRequestsStore` | `{ pending: MockRideRequest[], accept(id), decline(id) }` | Seeded with 2–3 mock pending requests (varied seat counts, mixed cash/gcash) — this is a session-simulation seed, same category as passenger's `mocks/drivers.ts`, not a persistent-record seed. `accept(id)` removes it from `pending` and calls `useTripStore.startTrip(request)`. |
...
Two categories, matching the distinction `apps/passenger` already draws between its two kinds of mock data:

- **Persistent-record data** (trip history, complaints, notifications, settlement log) starts **empty**. This is `apps/passenger`'s explicit "no invented sample content" rule (see `EmptyState`'s own doc comment) — these represent real records that a fresh account genuinely has none of yet.
- **Session-simulation data** (the pending requests queue) is seeded with 2–3 mock entries, the same category as passenger's `mocks/drivers.ts` — data that exists purely so an interactive flow (Accept/Decline → Active Trip) has something to demonstrate. Without this, the Requests/Active-Trip/History/Earnings chain could never be exercised end-to-end in this UI-only pass.

Documents start `'unsubmitted'` for all 4 types — a real starting state, not a mock convenience.
