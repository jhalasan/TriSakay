...
- `driver_profiles.user_id` is itself a FK → `users.id`, so a two-hop PostgREST embed (`trips` → `driver_profiles` → `users`) is well-supported for the driver's name without a new database object.
...
- Four independent `count: 'exact', head: true` queries, run with `Promise.all`:
  ...
  - `openComplaints`: `.from('complaints').select('*', { count: 'exact', head: true }).not('status', 'in', '(resolved,dismissed)')`
...
```
- `.from('trips').select('id, status, updated_at, driver_profiles(user_id, users(full_name))').order('updated_at', { ascending: false }).limit(limit)` — a two-hop PostgREST embed through the real `driver_id → driver_profiles.user_id → users.id` FK chain, not a view, so embedding is on solid ground here (unlike the two view-backed functions above).
...
- `listRecentTripActivity` maps the two-hop embed correctly, including a row where `driver_profiles`/`users` come back `null` (a trip whose driver row was deleted — degrades to `driverName: null`, doesn't crash).
