# TriSakay — locked designs, migration notes

Repo: `jhalasan/TriSakay` (branch `main`).

Locked, in handoff order:

| Design | Source of truth | Handoff bundle |
| --- | --- | --- |
| Homepages (passenger home + driver dashboard, two states each) | `TriSakay Home Final.dc.html` | — (anatomy below) |
| Passenger app, 36 screens | `TriSakay Passenger Screens.dc.html` | `design_handoff_trisakay_passenger/` |
| Driver app, 22 frames (splash + dashboard excluded) | `TriSakay Driver Screens.dc.html` | `driver handoff/` |
| Admin (PSO) portal, 14 frames | `TriSakay Admin Redesign.dc.html` | `design_handoff_trisakay_admin/` |

The driver app is **locked as of 2026-09-02**. Its four shared pages — profile, settings, notifications, log out — are the passenger screens with driver content substituted; implement them from the passenger code, not as new designs. Any structural divergence between the two apps on those pages is a bug. Details in `driver handoff/README.md`.

The admin portal is **locked as of 2026-09-09**. It is a web app (`apps/admin`, Vite + React + CSS Modules) and shares only colour values with the mobile apps — `packages/ui` cannot be imported there. Its redesign deliberately changes three things in the repo: the rail widens to 236px, the top bar to 62px, and `NAV_GROUPS` is regrouped into Operations / People / Casework / Insight / Administration. Full spec and phased Claude Code prompt in `design_handoff_trisakay_admin/`.

Homepage anatomy follows.
