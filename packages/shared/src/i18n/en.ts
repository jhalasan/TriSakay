export const en = {
  common: {
    gcash: 'GCash',
    cash: 'Cash',
    cancel: 'Cancel',
    delete: 'Delete',
    ok: 'OK',
  },
  offline: {
    stripMessage: 'No connection — showing your last update',
    title: "You're offline",
    cause: 'TriSakay needs data or Wi-Fi to find drivers near you. Nothing was lost — your last trip is still saved.',
    tryAgain: 'Try again',
    viewLastReceipt: 'View last ride receipt',
  },
  /** P1-25 (2026-09-15 launch audit): passenger counterpart to driver.accountSuspended, adapted for "request rides" instead of "accept rides". */
  accountSuspended: {
    suspendedTitle: 'Your account has been suspended',
    suspendedBody:
      'A PSO staff member has suspended your account. Visit the PSO office for details and next steps before you can request rides again.',
    deactivatedTitle: 'Your account has been deactivated',
    deactivatedBody: 'A PSO staff member has deactivated your account. Visit the PSO office for details and next steps.',
    psoOfficeLabel: 'PSO office',
    psoOfficeAddress: 'City Hall Annex, Barangay Dadiangas West',
    psoOfficeHours: 'Monday to Friday · 8:00 AM – 5:00 PM',
    refreshStatus: 'Refresh status',
    logOut: 'Log out',
  },
  home: {
    profileAccessibilityLabel: 'Profile',
    notificationsAccessibilityLabel: 'Notifications',
    ctaTitle: 'Request a Tricycle',
    ctaSubtitle: 'Book a ride in a few taps',
    heroTagline: 'Your ride is just a tap away.',
    confirmRideButton: 'Confirm ride',
    locationErrorTitle: "Couldn't get your location",
    locationErrorMessage: 'Try again, or set your pickup point by searching or dropping a pin on the map.',
    greetingMorning: 'Good morning',
    greetingAfternoon: 'Good afternoon',
    greetingEvening: 'Good evening',
    whereTo: 'Where to?',
    findingLocation: 'Finding your location…',
    dragPinToSetPickup: 'Drag the pin to set pickup',
    pickupAccessibilityLabel: 'Change pickup location',
    pickupFallback: 'Set pickup location',
    savedPlaces: 'Saved places',
    noSavedPlacesTitle: 'No saved places yet',
    noSavedPlacesMessage: 'Places you save will appear here for one-tap booking.',
    savedPlacesErrorTitle: "Couldn't load saved places",
    savedPlacesErrorMessage: 'Something went wrong. Try again later.',
    deleteSavedPlaceTitle: 'Remove this saved place?',
    deleteSavedPlaceMessage: 'This cannot be undone.',
    deleteSavedPlaceAccessibilityLabel: 'Remove saved place',
    statsTripsLabel: 'Trips',
    statsDiscountLabel: 'Discount',
    statsTripsEmptyValue: '0',
    statsDiscountEmptyValue: 'Not set',
    ctaFareChipPrefix: 'Fares from ₱25',
    ctaNearbySuffix: '{count} nearby',
    savedPlacesManage: 'Manage',
    discountLabelSeniorCitizen: 'Senior',
    discountLabelPwd: 'PWD',
    discountLabelStudent: 'Student',
  },
  savePlace: {
    title: 'Save this place',
    editTitle: 'Edit saved place',
    nameLabel: 'Name',
    namePlaceholder: 'e.g. Gym, Mom’s house',
    iconLabel: 'Icon',
    saveButton: 'Save place',
    saveChangesButton: 'Save changes',
    saveAccessibilityLabel: 'Save this place',
    savedAccessibilityLabel: 'Saved',
  },
  savedPlacesManagement: {
    eyebrow: 'Quick access',
    title: 'Saved places',
    intro: 'Saved places appear on your home screen for one-tap booking.',
    addFromMap: 'Save a place from the map',
    emptyTitle: 'No saved places yet',
    emptyMessage: 'Places you save from a trip will appear here.',
    editAccessibilityLabel: 'Edit {label}',
    removeAccessibilityLabel: 'Remove {label}',
    backAccessibilityLabel: 'Back',
  },
  accountPages: {
    fareDiscountTitle: 'Fare discount',
    fareDiscountBannerSuffix: 'off every fare',
    fareDiscountBannerSubtitle: 'Seniors, PWDs, and students with a valid ID',
    fareDiscountIntro:
      'Senior citizens, PWDs, and students may apply for a fare discount. Upload photos of the front and back of your valid ID — a PSO Supervisor checks its authenticity before the discount takes effect.',
    categorySenior: 'Senior',
    categoryPwd: 'PWD',
    categoryStudent: 'Student',
    categorySeniorFull: 'Senior Citizen',
    idFront: 'ID — front',
    idBack: 'ID — back',
    idFrontCapturedTitle: 'Front captured',
    idBackCapturedTitle: 'Back captured',
    idCapturedSubtitle: 'Tap to retake',
    idFrontEmptyTitle: 'Add the front',
    idBackEmptyTitle: 'Add the back',
    idEmptySubtitle: 'Hold the ID sideways to fill the frame',
    submitApplication: 'Submit application',
    discountDisclaimer: 'Your ID photo is only visible to you and to PSO Supervisor / Admin reviewers.',
    photoRequiredError: 'Add photos of both the front and back of your ID before submitting.',
    permissionNeededTitle: 'Permission needed',
    permissionNeededMessage: 'Allow photo library access to upload your ID.',
    statusApproved: 'Approved',
    statusPendingReview: 'Pending review',
    statusApprovedNote: 'Your discount is active — it applies automatically to every fare.',
    statusPendingNote: 'Submitted for PSO review. You will be notified once a decision is made.',
    statusRejectedBadge: 'Previous application rejected',
    reviewerNote: "Reviewer's note",
    submitNewApplicationNote: 'You can submit a new application below.',

    fareMatrixTitle: 'Fare matrix',
    regulatedUnder: 'Regulated under',
    defaultOrdinance: 'City tricycle fare ordinance',
    regularFare: 'Regular fare',
    firstKmPrefix: 'First',
    kmSuffix: 'km',
    everyKmAfter: 'Every km after',
    perKmSuffix: '/km',
    discountedFare: 'Discounted fare',
    discountedFareBodyPrefix: 'Approved Senior Citizen, PWD, and student riders get',
    discountedFareBodySuffix: 'off the regular fare, applied automatically once approved.',
    applyForFareDiscount: 'Apply for a fare discount',
    howFareEstimated: 'How your fare is estimated',
    howFareEstimatedBody:
      'The fare is charged per seat booked and based on the road distance between pickup and drop-off, following the rule above.',
    routeEstimateNotice:
      'The displayed route and distance are estimates. Actual tricycle routes may differ, which can affect the final fare confirmed at drop-off.',
    couldNotLoadFareMatrix: 'Could not load the fare matrix.',

    paymentMethodsTitle: 'Payment methods',
    gcash: 'GCash',
    gcashLinkedNote: 'Linked at checkout, not saved',
    cashDirectNote: 'Pay your driver directly',
    available: 'Available',
    paymentHistoryRowTitle: 'Payment history',
    savingNotAvailableNotice: "Saving a GCash account or card isn't available yet — you'll confirm the wallet at every checkout.",

    paymentHistoryTitle: 'Payment history',
    paidSuffix: 'paid',
    noPaymentsYetTitle: 'No payments yet',
    noPaymentsYetMessage: 'Your paid trips will show up here.',
    statusPaid: 'Paid',
    statusRefunded: 'Refunded',
  },
  settings: {
    title: 'Settings',
    tagline: 'Notifications, privacy, and language for this device.',
    sectionNotifications: 'Notifications',
    sectionPrivacy: 'Privacy',
    // Passenger-only consolidated entry point (Privacy & Safety Center); the
    // driver app's own Settings screen still uses sectionPrivacy/sectionLegal
    // and its own separate profile/safety.tsx below, untouched.
    sectionPrivacySafety: 'Privacy & Safety',
    privacySafetyCenterRow: 'Privacy & Safety Center',
    privacySafetyCenterRowSubtitle: 'Emergency SOS, location, and legal info',
    sectionPreferences: 'Preferences',
    sectionLegal: 'Legal',
    legalTermsPrivacy: 'Terms & Privacy',
    legalTermsPrivacySubtitle: 'Review what you agreed to at sign-up',
    legalSafetyCenter: 'Safety Center',
    legalSafetyCenterSubtitle: 'Emergency numbers and safety tips',
    sectionHelp: 'Help',
    replayWalkthrough: 'Replay welcome walkthrough',
    replayWalkthroughSubtitle: 'The intro slides shown on first launch',
    replayTour: 'Replay app tour',
    // P2 (UAT audit): this row actually restarts the in-app coach tutorial
    // (useTutorial), not the walkthrough above — the two are genuinely
    // different flows, and the old subtitle ("Take the welcome walkthrough
    // again") claimed this row did the other one's job.
    replayTourSubtitle: 'Point-by-point tips on the Home screen',
    pushNotifications: 'Push notifications',
    pushNotificationsSubtitle: 'Driver matched, arrival, and fare updates',
    smsReceipts: 'SMS receipts',
    smsReceiptsSubtitlePrefix: 'Sent to',
    emailReceipts: 'Email receipts',
    locationTracking: 'Location tracking',
    locationTrackingSubtitle: 'Required to request a tricycle',
    language: 'Language',
    languageEnglish: 'English',
    languageFilipino: 'Filipino',
    sectionAccount: 'Account',
    deactivateAccountRow: 'Deactivate account',
    deactivateAccountRowSubtitle: 'Close your TriSakay account',
    logOut: 'Log out',
    versionFooter: 'TriSakay v1.0 · General Santos City PSO',
  },
  profile: {
    eyebrow: 'Account',
    title: 'Profile',
    edit: 'Edit',
    save: 'Save',
    riderFallback: 'Rider',
    verifiedPassenger: 'Verified passenger',
    ridesSuffix: 'rides',
    discountActiveSuffix: 'discount active',
    applyForDiscountTitle: 'Apply for a fare discount',
    email: 'Email',
    phone: 'Phone',
    phonePlaceholder: '09XX XXX XXXX',
    firstNamePlaceholder: 'First name',
    lastNamePlaceholder: 'Last name',
    paymentMethods: 'Payment methods',
    fareMatrix: 'Fare matrix',
    settingsRow: 'Settings',
    couldNotSaveTitle: 'Could not save',
    photoPermissionTitle: 'Permission needed',
    photoPermissionMessage: 'Allow photo library access to change your profile picture.',
    couldNotUploadPhotoTitle: 'Could not upload photo',
    couldNotSavePhotoTitle: 'Could not save photo',
    tryAgainFallback: 'Please try again.',
  },
  confirm: {
    title: 'Confirm ride',
    noDestinationSelected: 'No destination selected yet.',
    chooseDestination: 'Choose a destination',
    pickupLocationLabel: 'Pickup Location',
    destinationLabel: 'Where to?',
    pickupEyebrow: 'Pickup',
    dropoffEyebrow: 'Drop-off',
    change: 'Change',
    pickupPointFallback: 'Pickup point',
    notSetYetFallback: 'Not set yet',
    routePreview: 'Route preview',
    seats: 'Seats',
    estimatedFare: 'Estimated fare',
    viewFareMatrix: 'View fare matrix',
    discountAppliedSuffix: '% discount applied',
    roadDistanceSuffix: 'km road distance',
    couldNotReachFareService: 'Could not reach the fare service.',
    estimatingFare: 'Estimating fare…',
    fareConfirmedAtDropoff: 'Final fare is confirmed at drop-off',
    applyForDiscountPrompt: 'Senior, PWD, or student? Apply for a fare discount',
    paymentMethod: 'Payment method',
    requestRide: 'Request ride',
    couldNotRequestRide: 'Could not request a ride. Please try again.',
  },
  findingDriver: {
    title: 'Finding a driver',
    searchingNearbyEyebrow: 'Searching nearby',
    lookingForTricycleTo: 'Looking for a tricycle to',
    lookingForTricycleNearby: 'Looking for a tricycle nearby',
    tricyclesNearby: '{count} tricycles nearby',
    fareHeld: 'Fare held at {fare} · {seats} seats',
    cancelRequest: 'Cancel request',
  },
  trip: {
    noDriverMatchedTitle: 'No driver matched',
    noDriverMatchedMessage: 'Try requesting a ride again.',
    backToHome: 'Back to Home',
    mapCaption: 'Map · trip route',
    driverAssigned: 'Driver assigned',
    tripInProgressCaption: 'Trip in progress',
    tripInProgress: 'Trip in progress',
    noInAppCallNotice: 'No in-app call or message — coordination is in person.',
    sosButton: 'Emergency SOS',
    sosCaption: "Press and hold if you're in immediate danger",
    cancelRide: 'Cancel ride',
    cancelRideTitle: 'Cancel this ride?',
    cancelRideMessage: "You won't be charged, and it won't count against your account.",
    cancelRideKeepIt: 'Keep ride',
    cancelRideConfirm: 'Cancel ride',
  },
  emergency: {
    title: 'Emergency',
    subtitle: "If you're in immediate danger, call for help now.",
    callButton: 'Call 911 / PNP',
    notifyingPso: 'Notifying PSO of your location…',
    notifiedPso: 'PSO has been notified of your location.',
    notifyFailed: "Couldn't reach PSO. Your call for help above still works.",
    retry: 'Try again',
    backToTrip: 'Back to trip',
    sentAtLabel: 'Sent at {time}',
    sharedWithPso: 'Shared with the PSO',
    nearLabel: 'Near',
  },
  legal: {
    title: 'Terms & Privacy',
    whatWeCollect: 'What we collect & share',
  },
  safety: {
    title: 'Safety Center',
    subtitle: 'Know what to do, and what TriSakay does, if something feels wrong.',
    callButton: 'Call 911 / PNP',
    howSosWorksTitle: 'How in-app SOS works',
    howSosWorksBody:
      "Hold the Emergency SOS button on your trip screen if you're in immediate danger during a ride — it alerts PSO with your live location and your driver's details. You can also send an SOS to PSO anytime from the Privacy & Safety Center, even without an active ride.",
    tipsTitle: 'Safety tips',
    tip1: 'Check that the plate number and driver photo shown in the app match the tricycle before boarding.',
    tip2: 'Keep your phone charged and location on during the trip so SOS can share where you are.',
    tip3: 'Let someone you trust know your pickup and drop-off before you ride.',
    reportIssueLink: 'Report a non-emergency issue',
  },
  privacySafety: {
    eyebrow: 'Privacy & Safety',
    title: 'Privacy & Safety Center',
    subtitle: 'Everything about staying safe and in control of your data, in one place.',
    sosButton: 'Send SOS to PSO',
    sosConfirmTitle: 'Send an SOS alert?',
    sosConfirmMessage:
      'This shares your live location with PSO right now, even without an active ride. Only use this if you feel unsafe.',
    sosConfirmButton: 'Send SOS',
    sosSending: 'Sending your location to PSO…',
    sosSent: 'PSO has been notified of your location.',
    sosFailed: 'Could not reach PSO. Try again, or call 911 directly.',
    sosLocationRequired: 'Location access is needed to share where you are. Enable it in system settings and try again.',
    retry: 'Retry',
    sectionAccountSecurity: 'Account security',
    changePasswordRow: 'Change password',
    changePasswordRowSubtitle: 'Update the password for this account',
    sectionLegal: 'Legal',
    termsPrivacyRow: 'Legal Policy',
    termsPrivacyRowSubtitle: 'Terms of Service & Privacy Policy',
    yourDataTitle: 'Your data',
    yourDataBody:
      'You may ask PSO to give you a copy of, correct, or delete your personal data at any time. Reach PSO through the contact channel on your ride receipts or at the barangay office.',
  },
  changePassword: {
    title: 'Change password',
    currentPassword: 'Current password',
    newPassword: 'New password',
    confirmNewPassword: 'Confirm new password',
    saveButton: 'Update password',
    notAvailableNotice:
      "Changing your password from the app isn't available yet in this prototype. Contact PSO if you need help accessing your account.",
  },
  noDriversNearby: {
    title: 'No drivers nearby',
    cause:
      "We asked every driver nearby, but nobody's free yet — this is common around peak hours.",
    notifyToggleLabel: 'Notify me when one is free',
    searchAgain: 'Search again',
    changePickupPoint: 'Change pickup point',
  },
  rideCancelled: {
    title: 'Ride cancelled',
    cancelledByDriverSuffix: "cancelled this ride. You weren't charged and nothing counts against your account.",
    cancelledGeneric: "This ride was cancelled. You weren't charged and nothing counts against your account.",
    fareHeld: 'Fare held',
    discountStillValid: 'Your discount is still valid',
    findAnotherDriver: 'Find another driver',
    reportCancellation: 'Report this cancellation',
  },
  payment: {
    title: 'Payment',
    amountDue: 'Amount due',
    tripTo: 'Trip to',
    payWith: 'Pay with',
    gcashWalletTitle: 'GCash Wallet',
    gcashWalletSubtitle: 'Pay using your GCash balance',
    cashSubtitle: 'Pay the driver directly',
    missingRideDetails: 'Missing ride details — please go back and try again.',
    waitingForCashConfirm: 'Waiting for the driver to confirm cash received…',
    waitingForGcashConfirm: 'Waiting for PayMongo to confirm your payment…',
    stillWaitingCashConfirm: 'Still waiting for the driver to confirm cash received.',
    paymentFailedRetry: 'Payment failed. You can retry.',
    couldNotStartGcashCheckout: 'Could not start GCash checkout.',
    couldNotConfirmPaymentYet: "We couldn't confirm your payment yet. You can retry.",
    checkAgain: 'Check again',
    retryGcash: 'Retry GCash',
    openingPaymongo: 'Opening PayMongo…',
    payNow: 'Pay now',
    keepScreenOpenNotice: 'Your ride is only marked paid once the payment is confirmed — keep this screen open.',
  },
  tripComplete: {
    title: 'Trip complete',
    subtitle: "You've arrived and your payment is settled.",
    fareLabel: 'Fare',
    paidViaLabel: 'Paid via',
    distanceLabel: 'Distance',
    referenceLabel: 'Trip ref.',
    continueButton: 'Continue',
  },
  rateDriver: {
    tripCompletedEyebrow: 'Trip completed',
    yourDriverFallback: 'Your driver',
    howWasYourRide: 'How was your ride?',
    paidVia: 'paid via',
    commentLabel: 'Comment (optional)',
    commentPlaceholder: 'Tell us about your trip',
    submitRating: 'Submit rating',
    skipForNow: 'Skip for now',
    couldNotConfirmDriver: "We couldn't confirm your driver for this trip — you can still continue.",
    continue: 'Continue',
  },
  history: {
    eyebrow: 'Your rides',
    title: 'Ride history',
    filterAll: 'All',
    filterCompleted: 'Completed',
    filterCancelled: 'Cancelled',
    thisMonth: 'This month',
    ridesSuffix: 'rides',
    rideSuffix: 'ride',
    completed: 'Completed',
    cancelled: 'Cancelled',
    noDriverAssigned: 'No driver assigned',
    emptyTitle: 'No rides yet',
    emptyMessage: 'Your first tricycle booking will show up here with its fare and receipt.',
    requestATricycle: 'Request a tricycle',
    detailTitle: 'Ride details',
    notFoundTitle: 'Ride not found',
    notFoundMessage: "This ride isn't in your history anymore.",
    totalFare: 'Total fare',
    discountAppliedSuffix: '% discount applied',
    discountAppliedGeneric: 'Discount applied',
    route: 'Route',
    pickup: 'Pickup',
    dropoff: 'Drop-off',
    driver: 'Driver',
    minutesSuffix: 'min',
    seatsSuffix: 'seats',
    bodyNoPrefix: 'Body no.',
    payment: 'Payment',
    baseFare: 'Base fare',
    discountLabel: 'Discount',
    total: 'Total',
    cancellationReason: 'Cancellation reason',
    tripReference: 'Trip ref.',
  },
  complaints: {
    title: 'Complaints',
    tagline: "We're here to help make things right.",
    relatedTrip: 'Related trip',
    selectAPastRide: 'Select a past ride',
    noPastRides: 'No past rides to report on yet.',
    notRelatedToARide: "Not related to a specific ride",
    notRelatedToARideSubtitle: "Your ride never showed up in your history? Use this instead.",
    category: 'Category',
    subject: 'Subject',
    subjectPlaceholder: "What's this about?",
    message: 'Message',
    messagePlaceholder: 'Describe what happened',
    evidenceOptional: 'Evidence (optional)',
    evidenceHintPrefix: 'Up to',
    evidenceHintSuffix: 'photos.',
    submitComplaint: 'Submit complaint',
    attachmentUploadFailedPrefix: 'Your complaint was filed, but the evidence photos could not be attached',
    submittedTitle: 'Complaint submitted',
    submittedMessage: 'Our team will review this and follow up if needed.',
    submitAnother: 'Submit another',
    yourComplaints: 'Your complaints',
    noComplaintsYet: "You haven't filed any complaints.",
    categoryFare: 'Fare dispute',
    categoryConduct: 'Driver conduct',
    categorySafety: 'Safety concern',
    categoryLowRating: 'Low rating',
    categoryVehicleCondition: 'Vehicle condition',
    categoryOther: 'Other',
    statusOpen: 'Open',
    statusUnderReview: 'Under review',
    statusEscalated: 'Escalated',
    statusMediationScheduled: 'Mediation scheduled',
    statusResolved: 'Resolved',
    statusDismissed: 'Dismissed',
    detailTitle: 'Complaint status',
    notFoundTitle: 'Complaint not found',
    notFoundMessage: "This complaint isn't in your account anymore.",
    referencePrefix: 'Complaint',
    filedPrefix: 'Filed',
    progress: 'Progress',
    stageReceivedTitle: 'Received',
    stageReceivedBody: 'Reference sent by SMS.',
    stageUnderReviewTitle: 'Under review by the operator',
    stageUnderReviewBody: 'The driver has 48 hours to respond.',
    stageResolutionTitle: 'Resolution',
    stageResolutionBody: 'You will get a decision and any refund here.',
  },
  notifications: {
    title: 'Notifications',
    markAllRead: 'Mark all read',
    tagline: 'Ride, payment, and PSO updates.',
    newSuffix: 'new',
    filterAll: 'All',
    filterUnread: 'Unread',
    filterRides: 'Rides',
    today: 'Today',
    yesterday: 'Yesterday',
    emptyTitle: "You're all caught up",
    emptyMessage: 'No notifications yet.',
  },
  setDestination: {
    locatingPin: 'Locating pin…',
    tapOrDragPin: 'Tap or drag the pin to drop a destination',
    searchForDestination: 'Search for a destination',
    searchResults: 'Search results',
    droppedPin: 'Dropped pin',
    searching: 'Searching…',
    noMatches: 'No matches',
    lookingForPlacesNearby: 'Looking for places nearby.',
    tryDifferentSearchTerm: 'Try a different search term, or drop a pin on the map instead.',
    typePlaceNameOrTapMap: 'Type a place name, or tap the map to drop a pin.',
    confirmDestination: 'Confirm destination',
  },
  setPickup: {
    tapOrDragPin: 'Tap or drag the pin to set pickup',
    searchForPickup: 'Search for a pickup point',
    useCurrentLocation: 'Use current location',
    locatingCurrentLocation: 'Locating…',
    confirmPickup: 'Confirm pickup',
  },
  /** P2 (2026-09-15 launch audit): passenger login/register/forgot-password/
   * logout were hardcoded English while the driver equivalents already use
   * t.driver.* — this closes that gap. Root-level (not nested under a role
   * key) since the passenger app, unlike the driver app, has no such
   * namespace elsewhere in this dictionary. */
  auth: {
    login: {
      welcomeBack: 'Welcome back',
      welcomeNew: 'Welcome to TriSakay',
      subtitleReturning: 'Log in to book your next ride.',
      subtitleFirstTime: 'Log in to book your first ride.',
      mobileNumber: 'Mobile number',
      email: 'Email',
      password: 'Password',
      forgotPassword: 'Forgot password?',
      logIn: 'Log in',
      or: 'OR',
      createAccount: 'Create account',
      enterValidEmail: 'Enter a valid email address.',
      enterValidMobile: 'Enter a valid 10-digit mobile number.',
      passwordMinLength: 'Password must be at least 6 characters.',
      mobileComingSoon: 'Signing in with a mobile number is coming soon — use Email for now.',
    },
    register: {
      stepTitleAccount: 'Create account',
      stepTitleLegal: 'Terms & Privacy',
      stepLabel: 'Step {step} of 2',
      changePhotoAccessibilityLabel: 'Change profile photo',
      addPhotoAccessibilityLabel: 'Add a profile photo',
      tapToChangePhoto: 'Tap to change photo',
      addPhotoOptional: 'Add a profile photo (optional)',
      permissionNeededTitle: 'Permission needed',
      permissionNeededMessage: 'Allow photo library access to set a profile picture.',
      firstName: 'First name',
      firstNamePlaceholder: 'Juan',
      lastName: 'Last name',
      lastNamePlaceholder: 'Dela Cruz',
      email: 'Email',
      emailPlaceholder: 'you@example.com',
      phone: 'Phone number',
      phonePlaceholder: '09XX XXX XXXX',
      password: 'Password',
      confirmPassword: 'Confirm password',
      next: 'Next',
      enterFirstName: 'Enter your first name.',
      enterLastName: 'Enter your last name.',
      enterValidEmail: 'Enter a valid email address.',
      enterContactNumber: 'Enter a contact number.',
      enterValidContactNumber: 'Enter a valid PH mobile number (e.g. 09XX XXX XXXX).',
      passwordMinLength:
        'Password must be at least 10 characters and include upper and lower case letters plus a number or symbol.',
      passwordsDoNotMatch: 'Passwords do not match.',
      acceptTermsIntro: 'Please read and accept these before your account is created.',
      versionLabel: 'Terms {tos} · Privacy {privacy}',
      whatWeCollect: 'What we collect & share',
      acceptTerms: 'I have read and accept the Terms of Service and Privacy Policy',
      createAccountButton: 'Create account',
      couldNotReadPhoto: 'Could not read the selected photo.',
      accountCreatedTitle: 'Account created',
      accountCreatedPhotoFailedMessage:
        "Your account is ready, but the profile photo didn't upload: {error}. You can add it later from Profile.",
      checkEmailTitle: 'Check your email',
      checkEmailMessage: 'We sent a confirmation link to {email}. Confirm it, then log in.',
    },
    forgotPassword: {
      title: 'Reset password',
      intro: "Enter the email on your account and we'll send you a code to reset your password.",
      email: 'Email',
      sendCode: 'Send code',
      enterValidEmail: 'Enter a valid email address.',
    },
    logout: {
      title: 'Log out?',
      message: "You'll need to log in again to book a ride.",
      confirm: 'Log out',
    },
    deactivateAccount: {
      title: 'Deactivate your account?',
      message:
        "You won't be able to book rides until you visit the PSO office to reactivate. This does not delete your ride history.",
      confirm: 'Deactivate account',
      errorFallback: "Couldn't deactivate your account. Please try again.",
    },
  },
  driver: {
    login: {
      welcomeBack: 'Welcome back',
      subtitle: 'Log in to start driving.',
      driverChip: 'Driver',
      mobileNumber: 'Mobile number',
      email: 'Email',
      password: 'Password',
      forgotPassword: 'Forgot password?',
      logIn: 'Log in',
      or: 'OR',
      registerAsDriver: 'Register as driver',
      enterValidEmail: 'Enter a valid email address.',
      enterValidMobile: 'Enter a valid 10-digit mobile number.',
      passwordMinLength: 'Password must be at least 6 characters.',
      mobileComingSoon: 'Signing in with a mobile number is coming soon — use Email for now.',
    },
    register: {
      stepTitleAccount: 'Register as driver',
      stepTitleDocuments: 'Documents & tricycle',
      stepLabel: 'Step {step} of 2',
      firstName: 'First name',
      firstNamePlaceholder: 'Juan',
      lastName: 'Last name',
      lastNamePlaceholder: 'Dela Cruz',
      email: 'Email',
      emailPlaceholder: 'you@example.com',
      phone: 'Phone number',
      phonePlaceholder: '09XX XXX XXXX',
      password: 'Password',
      confirmPassword: 'Confirm password',
      next: 'Next',
      enterFirstName: 'Enter your first name.',
      enterLastName: 'Enter your last name.',
      enterValidEmail: 'Enter a valid email address.',
      enterContactNumber: 'Enter a contact number.',
      enterValidContactNumber: 'Enter a valid PH mobile number (e.g. 09XX XXX XXXX).',
      passwordMinLength: 'Password must be at least 10 characters and include upper and lower case letters plus a number or symbol.',
      passwordsDoNotMatch: 'Passwords do not match.',
      documentsIntro: 'Upload these so a PSO reviewer can verify your franchise before you go online. All 4 are required. Accepted: a clear photo of each document, from your gallery.',
      plateNumber: 'Tricycle plate number',
      plateNumberPlaceholder: 'e.g. GSC-1187',
      enterPlateNumber: 'Enter your tricycle plate number.',
      acceptTermsIntro: 'Please read and accept these before your account is created.',
      whatWeCollect: 'What we collect & share',
      acceptTerms:
        'I have read and accept the Terms of Service and Privacy Policy, and confirm I am a licensed tricycle driver',
      registerButton: 'Register',
      checkEmailTitle: 'Check your email',
      checkEmailMessage:
        'We sent a confirmation link to {email}. Confirm it, then log in and upload your documents from there.',
      documentsSubmittedTitle: 'Documents submitted',
      documentsSubmittedMessage:
        "Your documents are under review. We'll send you a text message or email once your account is approved to go online.",
      registeredButFailedTitle: 'Registered, but documents failed to upload',
      registeredButFailedMessage: '{error}\n\nPlease try registering again so your documents are on file for review.',
      couldNotReadFile: 'Could not read one of your selected files.',
    },
    forgotPassword: {
      title: 'Reset password',
      intro: "Enter the email on your account and we'll send you a code to reset your password.",
      email: 'Email',
      sendCode: 'Send code',
      enterValidEmail: 'Enter a valid email address.',
    },
    resetPassword: {
      title: 'Reset password',
      noEmailFound: "We couldn't find the email for this reset. Please start again from the login screen.",
      backToLogin: 'Back to login',
      code: 'Code',
      newPassword: 'New password',
      confirmNewPassword: 'Confirm new password',
      resendCode: 'Resend code',
      sending: 'Sending…',
      resetPasswordButton: 'Reset password',
      introWithEmail: 'Enter the code we sent to {email} and choose a new password.',
      enterCode: 'Enter the code we emailed you.',
      passwordMinLength: 'Password must be at least 10 characters and include upper and lower case letters plus a number or symbol.',
      passwordsDontMatch: "Passwords don't match.",
      codeInvalid: 'That code is invalid or has expired. Request a new one below.',
      couldNotUpdatePassword: "Couldn't update your password. Request a new code and try again.",
      passwordUpdatedTitle: 'Password updated',
      passwordUpdatedBody: "You're signed in with your new password.",
      continueButton: 'Continue',
    },
    consent: {
      title: 'Before you drive',
      checkboxLabel: 'I have read and accept the Terms of Service and Privacy Policy',
      acceptAndContinue: 'Accept & Continue',
      notYouLogOut: 'Not you? Log out',
      whatWeCollect: 'What we collect & share',
    },
    locationPermission: {
      title: 'Turn on location',
      body: 'TriSakay needs your location to match you with nearby drivers and estimate pickup accurately.',
      blockedNote: 'Location is off for TriSakay in your device settings. Open settings to turn it on.',
      enableLocation: 'Enable location',
      openSettings: 'Open settings',
      notNow: 'Not now',
    },
    accountSuspended: {
      suspendedTitle: 'Your account has been suspended',
      suspendedBody:
        'A PSO staff member has suspended your account. Visit the PSO office for details and next steps before you can accept rides again.',
      deactivatedTitle: 'Your account has been deactivated',
      deactivatedBody: 'A PSO staff member has deactivated your account. Visit the PSO office for details and next steps.',
      psoOfficeLabel: 'PSO office',
      psoOfficeAddress: 'City Hall Annex, Barangay Dadiangas West',
      psoOfficeHours: 'Monday to Friday · 8:00 AM – 5:00 PM',
      refreshStatus: 'Refresh status',
      logOut: 'Log out',
    },
    logout: {
      title: 'Log out?',
      message: "You'll need to log in again to go online.",
      confirm: 'Log out',
    },
    notFound: {
      title: 'Page not found',
      message: "This screen doesn't exist.",
      backToHome: 'Back to Home',
    },
    splash: {
      subtitle: 'Your next ride starts here',
      driverBadge: 'Driver',
    },
    dashboard: {
      driverFallback: 'Driver',
      notificationsAccessibilityLabel: 'Notifications',
      online: 'You are online',
      offline: 'You are offline',
      onlineBadge: 'Online',
      offlineBadge: 'Offline',
      earningsToday: 'Earnings today',
      tripsToday: 'Trips today',
      rating: 'Rating',
      acceptRate: 'Accept rate',
      incomingRequest: 'Incoming request',
      listeningForRequests: 'Listening for ride requests…',
      locationError: 'Could not get your location. Make sure location services are turned on.',
      goOnline: 'Go online',
      offlineNote: "You won't receive ride requests while offline. Go online to start listening.",
      earningsTodayEyebrow: 'EARNINGS TODAY',
      statTrips: '{count} trips',
      statRating: '{rating}',
      statAcceptance: '{percent}% accepted',
      noRatingsYet: 'No ratings',
      pso: 'PSO verified',
      pendingVerification: 'Verification pending',
      bodyNoPrefix: 'Body no.',
      listeningTitle: 'Listening for requests',
      listeningMessage: "You're online. The next ride will appear right here.",
      listeningEyebrow: 'LISTENING',
      incomingRequestEyebrow: 'INCOMING REQUEST',
      requestExpired: 'expired',
    },
    requests: {
      title: 'Ride requests',
      eyebrow: 'Incoming',
      availableNow: 'Available now',
      requestSuffix: 'request',
      requestsSuffix: 'requests',
      offline: 'Offline',
      noRequestsTitle: 'No requests right now',
      noRequestsMessage: 'New ride requests will appear here.',
      offlineTitle: "You're offline",
      offlineMessage: 'Go online from the Dashboard to start receiving requests.',
      goToDashboard: 'Go to Home',
    },
    notifications: {
      title: 'Notifications',
      markAllRead: 'Mark all read',
      emptyTitle: "You're all caught up",
      emptyMessage: 'No notifications yet.',
      tagline: 'Stay updated with your trip activity',
      newCount: '{count} new',
      filterAll: 'All',
      filterUnread: 'Unread',
      filterRides: 'Rides',
      filterAccessibilityLabel: 'Filter: {label}',
      justNow: 'Just now',
      minuteAgo: '{count} minute ago',
      minutesAgo: '{count} minutes ago',
      hourAgo: '{count} hour ago',
      hoursAgo: '{count} hours ago',
      dayAgo: '{count} day ago',
      daysAgo: '{count} days ago',
    },
    verificationPending: {
      pendingTitle: 'Your account is under review',
      pendingBody:
        "We've received your registration and documents. Visit the PSO office with your original license, OR/CR, and franchise so a staff member can verify them in person. You'll get a text message or email once your account is approved and you can start accepting rides.",
      rejectedTitle: "Your verification wasn't approved",
      rejectedBody: 'Visit the PSO office for next steps on your driver verification.',
      rejectionReasonLabel: 'Reason given by PSO',
      refreshStatus: 'Refresh status',
      logOut: 'Log out',
      unsubmittedTitle: 'Finish your registration',
      unsubmittedBody:
        "Confirming your email created your account, but your documents haven't been submitted yet. Upload them below so a PSO reviewer can verify your franchise.",
      plateNumber: 'Tricycle plate number',
      plateNumberPlaceholder: 'e.g. GSC-1187',
      enterPlateNumber: 'Enter your tricycle plate number.',
      submitDocuments: 'Submit documents',
      documentsSubmittedTitle: 'Documents submitted',
      documentsSubmittedBody:
        "Your documents are under review. We'll send you a text message or email once your account is approved to go online.",
      couldNotReadFile: 'Could not read one of your selected files.',
    },
    history: {
      title: 'Trip history',
      eyebrow: 'Your trips',
      filterAll: 'All',
      filterDone: 'Done',
      filterCancelled: 'Cancelled',
      thisMonth: 'This month',
      tripSuffix: 'trip',
      tripsSuffix: 'trips',
      passengerFallback: 'Passenger',
      done: 'Done',
      emptyTitle: 'No trips yet',
      emptyMessage: 'Your completed trips will show up here.',
      detailTitle: 'Trip details',
      notFoundTitle: 'Trip not found',
      notFoundMessage: "This trip isn't in your history anymore.",
      totalFare: 'Total fare',
      passenger: 'Passenger',
      tripReference: 'Trip ref.',
    },
    earnings: {
      title: 'Earnings',
      totalTracked: 'Total earnings (tracked)',
      tripsCount: '{count} trips',
      averageFare: '{amount} average',
      last7Days: 'Last 7 days',
      today: 'Today',
      noEarningsTitle: 'No earnings yet',
      noEarningsMessage: 'Completed, paid rides will show up here by day.',
      peakHoursLabel: 'Busiest hours (last 30 days)',
      peakHoursCaption: 'Based on completed rides city-wide — use it to decide when to go online.',
      noPeakHoursTitle: 'Not enough data yet',
      noPeakHoursMessage: 'Peak-hour trends will show up here once more rides complete.',
    },
    profile: {
      title: 'Profile',
      accountEyebrow: 'Account',
      email: 'Email',
      phone: 'Phone',
      tricycle: 'Tricycle',
      verified: 'Verified',
      verifiedDriverRating: 'Verified driver · {rating} ({count} ratings)',
      ratingOnly: '{rating} ({count} ratings)',
      myRatings: 'My Ratings',
      complaints: 'Complaints',
      settings: 'Settings',
      accountSection: 'Account',
      driverFallback: 'Driver',
      editProfile: 'Edit profile',
      edit: 'Edit',
      saveChanges: 'Save changes',
      changePhoto: 'Change profile photo',
      viewMyRatings: 'View my ratings',
      permissionNeededTitle: 'Permission needed',
      permissionNeededMessage: 'Allow photo library access to change your profile picture.',
      couldNotSaveTitle: 'Could not save',
      couldNotUploadPhotoTitle: 'Could not upload photo',
      couldNotSavePhotoTitle: 'Could not save photo',
      pleaseTryAgain: 'Please try again.',
      phonePlaceholder: '09XX XXX XXXX',
      franchiseVerified: 'Franchise verified',
      franchiseClearedNoDate: 'PSO-cleared to operate',
      franchiseClearedWithDate: 'PSO-cleared to operate · valid to {date}',
    },
    ratings: {
      title: 'My Ratings',
      eyebrow: 'Passenger feedback',
      filterAll: 'All',
      filterWithComments: 'With comments',
      filterLow: 'Low',
      emptyTitle: 'No ratings yet',
      emptyMessage: 'Feedback from completed rides will show up here.',
      ratingsCount: '{count} ratings',
    },
    settings: {
      subtitle: 'Notifications, privacy, and language for this device.',
      pushNotificationsSubtitle: 'New ride requests and trip updates',
      smsReceiptsSubtitle: 'Sent to {phone}',
      emailReceiptsSubtitle: '{email}',
      locationTrackingSubtitle: 'Only while online or on a trip',
      versionLine: 'TriSakay Driver v1.0 · General Santos City PSO',
    },
    safety: {
      howSosWorksBody:
        "Hold the Emergency SOS button on your trip screen if a passenger situation turns dangerous during a trip — it alerts PSO with your live location, and your passenger's details when exactly one is aboard. You can also send an SOS to PSO anytime from the Privacy & Safety Center, even without an active trip.",
      tip1: 'Confirm the passenger matches the name and pickup shown in the app before starting the trip.',
      tip2: 'Keep your phone charged and location on while online or on a trip so SOS can share where you are.',
      tip3: 'PSO reviews every verified misconduct or safety complaint and can suspend an account over it.',
    },
    tripActive: {
      passengerFallback: 'Passenger',
      inProgress: 'In progress',
      navigate: 'Navigate',
      onlineNoPassengers: 'Online — no passengers',
      noPassengersNote: 'No passengers right now — stay online to keep getting requests, or end your trip below.',
      compatibleRequest: 'Compatible request',
      confirmCashReceived: 'Confirm cash received',
      gcashAutoConfirmed: 'GCash payment confirms automatically.',
      start: 'Start',
      complete: 'Complete',
      cancelPassengerTitle: 'Cancel this passenger?',
      cancelPassengerMessage: "This passenger's ride will be logged as cancelled.",
      completePassengerTitle: 'Complete this leg?',
      completePassengerMessage: 'This marks the trip complete for this passenger and finalizes their fare. This cannot be undone from here.',
      keep: 'Keep',
      cancelRide: 'Cancel ride',
      endTrip: 'End trip',
      endTripTitle: 'End your trip?',
      endTripMessage: "You'll stop being matched with new requests until you go online again.",
      stayOnline: 'Stay online',
      seatsSingular: 'seat',
      seatsPlural: 'seats',
      aboard: 'Aboard',
      passengerSingular: 'passenger',
      passengerPlural: 'passengers',
      ongoingStatus: 'Ongoing',
      gcashConfirmedInline: 'GCash confirmed',
    },
    complaints: {
      title: 'Complaints',
      subtitle: "We're here to help make things right.",
      new: 'New',
      category: 'Category',
      relatedTrip: 'Related trip (optional)',
      selectPastTrip: 'Select a past trip',
      noTripsYet: 'No past trips to report on yet.',
      subject: 'Subject',
      subjectPlaceholder: "What's this about?",
      message: 'Message',
      messagePlaceholder: 'Describe what happened',
      evidence: 'Evidence (optional)',
      evidenceHint: 'Up to {count} photos.',
      removePhotoAccessibilityLabel: 'Remove photo',
      addEvidencePhotoAccessibilityLabel: 'Add evidence photo',
      submit: 'Submit complaint',
      noComplaintsTitle: 'No complaints',
      noComplaintsMessage: 'Complaints you submit will appear here.',
      passengerFallback: 'Passenger',
      attachmentWarning: 'Your complaint was filed, but the evidence photos could not be attached ({error}).',
      categoryFareDispute: 'Fare dispute',
      categoryConduct: 'Passenger conduct',
      categorySafety: 'Safety concern',
      categoryLowRating: 'Low rating',
      categoryVehicleCondition: 'Vehicle condition',
      categoryOther: 'Other',
      statusOpen: 'Open',
      statusReview: 'Review',
      statusClosed: 'Closed',
    },
    tabs: {
      dashboard: 'Home',
      requests: 'Requests',
      history: 'History',
      earnings: 'Earnings',
      profile: 'Profile',
    },
    documents: {
      driversLicense: "Driver's license",
      driversLicenseHint: 'Must be a Professional license (Code A or A1)',
      orCr: 'OR / CR',
      franchisePermit: 'Franchise / permit',
      tricyclePhoto: 'Tricycle photo',
      statusUnsubmitted: 'Not uploaded',
      statusSelected: 'Selected',
      statusPending: 'Pending review',
      statusVerified: 'Verified',
      statusRejected: 'Rejected',
      permissionNeededTitle: 'Permission needed',
      permissionNeededMessage: 'Allow photo library access to upload this document.',
      removeTitle: 'Remove this photo?',
      removeMessage: "You'll need to upload it again before submitting.",
      removeConfirm: 'Remove',
      upload: 'Upload',
      removeAccessibilityLabel: 'Remove {label}',
      uploadAccessibilityLabel: 'Upload {label}',
    },
    requestCard: {
      decline: 'Decline',
      accept: 'Accept ride',
      newRideRequest: 'New ride request',
      seatsSingular: 'seat',
      seatsPlural: 'seats',
      pickupLabel: 'Pickup',
      dropoffLabel: 'Drop-off',
      pickupAwaySuffix: 'm away',
      paymentMethodCash: 'Cash',
      paymentMethodGcash: 'GCash',
    },
    errors: {
      availabilityUpdateFailed: "Couldn't update your availability. Please try again.",
      cashConfirmFailed: "Couldn't confirm the cash payment. Please try again.",
      startPassengerFailed: "Couldn't start this passenger's ride. Please try again.",
      completePassengerFailed: "Couldn't complete this passenger's ride. Please try again.",
      cancelPassengerFailed: "Couldn't cancel this passenger's ride. Please try again.",
      endTripFailed: "Couldn't end the trip. Please try again.",
      endTripBlocked: 'Complete or cancel every passenger before ending the trip.',
      acceptRideFailed: "Couldn't accept this ride. Please try again.",
    },
  },
} as const;
